...
private:
  ...
  public:
    storage(std::size_t stack_size, std::size_t next_size, std::size_t max_size)
      ...
      assert(traits_type::is_unbounded() ||
...
public:
  ...
  basic_pooled_fixedsize_stack(
      ...
      std::size_t next_size = 32, std::size_t max_size = 0) noexcept
      : storage_(new storage(stack_size, next_size, max_size)) {}

  stack_context allocate() { return storage_->allocate(); }

  void deallocate(stack_context &sctx) noexcept { storage_->deallocate(sctx); }
};
