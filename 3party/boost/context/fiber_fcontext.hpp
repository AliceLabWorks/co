...
public:
  manage_exception_state() { exception_state_ = *__cxa_get_globals(); }
  ~manage_exception_state() { *__cxa_get_globals() = exception_state_; }
...
template <typename Rec> transfer_t fiber_exit(transfer_t t) noexcept {
  ...
  std::size_t ss_size =
      *((unsigned long *)(reinterpret_cast<uintptr_t>(rec) - 16));
  long unsigned int ss_base =
      *((unsigned long *)(reinterpret_cast<uintptr_t>(rec) - 8));
...
fcontext_t create_fiber1(StackAlloc &&salloc, Fn &&fn) {
  ...
  // create shadow stack
  void *ss_base = (void *)syscall(__NR_map_shadow_stack, 0, ss_size,
  ...
  *((unsigned long *)(reinterpret_cast<uintptr_t>(storage) - 8)) =
...
public:
  ...
  ~fiber() {
    if (__builtin_expect(!!(nullptr != fctx_), 0)) {
