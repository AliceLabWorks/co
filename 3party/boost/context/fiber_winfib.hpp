...
#else
    fiber = ::ConvertThreadToFiber(nullptr);
    if (unlikely(nullptr == fiber)) {
      assert(ERROR_ALREADY_FIBER == ::GetLastError());
      fiber = ::GetCurrentFiber();
      assert(nullptr != fiber);
      assert(reinterpret_cast<LPVOID>(0x1E00) != fiber);
    }
#endif
  }
...
public:
  ...
  void run() {
    ...
    force_unwind = false;
    std::move(c).resume();
    assert_MSG(false, "fiber already terminated");
  }
};
...
static fiber_activation_record *create_fiber1(StackAlloc &&salloc, Fn &&fn) {
  ...
  assert((sizeof(capture_t)) < sctx.size);
...
static fiber_activation_record *create_fiber2(preallocated palloc,
  ...
  assert((sizeof(capture_t)) < palloc.size);
  // reserve space for control structure
  void *storage =
...
public:
  ...
  ~fiber() {
    if (unlikely(nullptr != ptr_) && !ptr_->main_ctx) {
...
#endif
    if (unlikely(detail::fiber_activation_record::current()->force_unwind)) {
